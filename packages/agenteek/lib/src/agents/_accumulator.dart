...
class AgentResponseAccumulator {
  ...
  String get currentOutput => _finalOutputBuffer.toString();
  String get currentThinking => _finalThinkingBuffer.toString();
  ...
  void add(ChatResult<String> result) {
    ...
    if (result.output.isNotEmpty) {
      ...
      streamingOutput?.add(_finalOutputBuffer.toString());
    }

    // Accumulate thinking text
    if (result.thinking != null) {
      _finalThinkingBuffer.write(result.thinking);
      streamingThinkingOutput?.add(_finalThinkingBuffer.toString());
    }
